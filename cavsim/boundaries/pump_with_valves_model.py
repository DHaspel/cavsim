...
class PumpSuctionValve(BaseBoundary):
    ...
    def initialize(self) -> None:
        ...
        self.field('pump_pressure')[:, :] = self.fluid.initial_pressure * np.ones(self.field('pump_pressure').shape)[:, :]
    ...
    def calculate_pump_pressure(self, compressibility, density_pump):
        ...
        self.pump_pressure[0, 0] = self._pump_volume_change[0, 0] / (self.pump_death_volume + self._pump_volume[1, 0])
    ...
    def calculate_discharge_gap_flow(self, pressure, density):
        ...
        self.discharge_mass_flow[0, 0] = result * density
    ...
    def calculate_suction_gap_flow(self, pressure, density):
        ...
        gap_velocity = (np.sqrt(np.abs(pressure - self.pump_pressure[0, 0]) * 2.0
                                / (self._suction_valve_zeta[1, 0] * density))
                        * np.sign(pressure - self.pump_pressure[0, 0]))
        result = gap_velocity * self._suction_gap_area[1, 0]
        self.mass_flow_suction_valve[0, 0] = result * density
