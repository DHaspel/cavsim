...
class LeftBoundaryPressure(BaseBoundary):
    ...
    def initialize(self) -> None:
        ...
        self._pressure_b = self._boundary
        self._velocity_b = 0.0
        self._delta_x_right = self.right.value(Measure.deltaX)
        self.delta_x_b = self.right.value(Measure.deltaX)
    ...
    def _calculate_space_interpolation(self) -> None:
        ...
        velocity_right = self._velocity[1, 0]
        pressure_right = self._pressure[1, 0]
        velocity_b = self._velocity[1, 1]
        pressure_b = self._pressure[1, 1]
        speed_of_sound_b = self._sos[1, 0]
