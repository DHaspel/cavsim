...
class PumpSuctionValve(BaseBoundary):
    ...
    def initialize(self) -> None:
        ...
        self.field('pressure')[:, :] = self.left.value(Measure.pressureLast) * np.ones(self.field('pressure').shape)[:, :]
    ...
    def calculate_suction_gap_reynolds_number(self, displacement, volume_flow, viscosity):
        ...
        if self._suction_gap_area[1, 0] == 0.0:
            gap_reynolds_number = 0.0
        else:
    ...
    def calculate_suction_flow_force(self, dimensionless_coefficient, density, lower_pressure, upper_pressure):
        ...
        result = dimensionless_coefficient * (self._pressure[1, 1] - self.pump_pressure[0, 0]) * (self.suction_outer_diameter**2) * np.pi / 4.0
    ...
    def calculate_suction_zeta_value(self, displacement, reynolds_number, lower_pressure, upper_pressure, density):
        ...
        #self._suction_delta_p[0, 0] = self.pump_pressure[1, 1] - self.pump_pressure[0, 0]
    ...
    def calculate_pump_pressure(self, bulk_modulus, density_pump, pressure):
        ...
        suction_volume_flow = self.calculate_suction_gap_flow(pressure, density_pump)
        self.mass_flow_suction_valve[0, 0] = suction_volume_flow * density_pump


    def calculate_discharge_gap_flow(self, pressure, density):

        self._discharge_gap_flow[0, 0] = (np.sqrt(np.abs(pressure-self.pump_pressure[0, 0]) * 2.0
                                / (self._discharge_valve_zeta[2, 0] * density)))
        result = self._discharge_gap_flow[0, 0]
        result = 0.0

        return result

    def calculate_suction_gap_flow(self, pressure, density):
        if self._suction_gap_area[2, 0] == 0.0:
            self._suction_valve_zeta[2, 0] = 1e9

        result = (np.sqrt(np.abs(pressure - self.pump_pressure[0, 0]) * 2.0
                                / (self._suction_valve_zeta[2, 0] * density)))
        if self._suction_gap_area[2, 0] == 0.0:
            result = 0.0
        self._suction_gap_flow[0, 0] = result * self._suction_gap_area[2, 0]
        result = self._suction_gap_flow[0, 0]
    ...
    def calculate_next_inner_iteration(self, iteration: int) -> bool:
        ...
        self.calculate_pump_pressure(bulk_modulus, density_pump, pressure_a)
