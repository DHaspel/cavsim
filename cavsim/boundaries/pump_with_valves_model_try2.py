...
class PumpSuctionValve(BaseBoundary):
    ...
    def __init__(
            ...
            initial_discharge_pressure: float,          # Pa
        ...
        self._initial_discharge_pressure = initial_discharge_pressure
        ...
        self.initial_p0 = 1.28e5
        self.mass_fraction_air = 0.0e-4
        self.density_air = 1.293
        self.density_air_new = 1.293
        self.initial_V0 = 1.0
        self.cavitation_counter = False
        ...
        self._alpha_air = self.field_create('alpha', 3)
        self._gas_constant = 287.1
        self._temperature = 293.15
        self._pump_volume_velocity = self.field_create('pump_volume_velocity', 3)
        self._pump_density = self.field_create('density', 3)
        self._pump_sos = self.field_create('sos', 3)
        self._alpha_vapor = self.field_create('alpha_vapor', 3)
    ...
    @property
    def initial_discharge_pressure(self):
        """

        :return:
        """
        return self._initial_discharge_pressure
    ...
    def calculate_density_air(self):

        self.density_air = self.pump_pressure[1, 0]/(self._gas_constant * self._temperature)

        return None

    def calculate_density_air(self):

        self.density_air_new = self.pump_pressure[0, 0]/(self._gas_constant * self._temperature)

        return None

    def calculate_alpha_air(self):
        self._alpha_air[1, 0] = (self.mass_fraction_air * self.fluid.density(pressure=self.pump_pressure[1, 0])
                                / (self.density_air
                                   - self.mass_fraction_air * self.density_air
                                   + self.mass_fraction_air * self.fluid.density(pressure=self.pump_pressure[1, 0])))
        return None

    def calculate_alpha_air_new(self):
        self._alpha_air[0, 0] = (self.mass_fraction_air * self.fluid.density(pressure=self.pump_pressure[1, 0])
                                / (self.density_air_new
                                   - self.mass_fraction_air * self.density_air_new
                                   + self.mass_fraction_air * self.fluid.density(pressure=self.pump_pressure[1, 0])))
        return None
    ...
    def initialize(self) -> None:
        ...
        self.field('discharge_pressure')[:, :] = self.initial_discharge_pressure * np.ones(self.field('discharge_pressure').shape)[:, :]
        self.field('friction')[:, :] = np.zeros(self.field('friction').shape)[:, :]
        self.field('speed_of_sound')[:, :] = np.ones(self.field('friction').shape)[:, :]
        self.field('pump_pressure')[:, :] = self.initial_pressure * np.ones(self.field('pump_pressure').shape)[:, :]
        ...
        self.field('suction_spring_force')[:, :] = (np.ones(self.field('suction_spring_force').shape)[:, :]
                                                    * self._suction_spring_force0)
        self.field('suction_gravity_force')[:, :] = np.ones(self.field('suction_gravity_force').shape)[:, :]
        ...
        self.field('alpha_vapor')[:, :] = np.zeros(self.field('alpha_vapor').shape)[:, :]
    ...
    def calculate_suction_contact_pressure(self, lower_pressure,
        ...
        f1 = 1.0 * pk * self.suction_radius * np.pi
    ...
    def calculate_suction_dimensionless_coefficient_of_force(self, reynolds_number, displacement):
        if reynolds_number == 0.0:
            ...
            result = (self._suction_factor_k0
                      ...
                      - self._suction_factor_k2 * np.log(np.abs(reynolds_number)))
    ...
    def calculate_suction_zeta_value(self, displacement, reynolds_number, lower_pressure, upper_pressure, density):
        ...
        self._suction_delta_p[0, 0] = ((self.mass_flow_suction_valve[0, 0] / (density * self._suction_gap_area[1, 0]))**2) * density / 2.0 * result * np.sign(self.mass_flow_suction_valve[0, 0])
        if np.logical_or(np.sign(self.mass_flow_suction_valve[0, 0]) <= 0.0, displacement <= 3e-6):
            self._suction_delta_p[0, 0] = 0.0
        #if self._suction_delta_p[0, 0] >= 1.0e5:
        #    self._suction_delta_p[0, 0] = 1.0e5
    ...
    def calculate_suction_flow(self, no_flow):
        ...
        area_a = self._area[0]
        sos_a = self._sos[1, 0]
        if self.cavitation_counter:
            density_a = self.fluid.density(pressure=pressure_a, temperature=None)
            #self._pressure[0, 1] = self.fluid.vapor_pressure()
            #velocity = velocity_a - friction_a * self._delta_t - 1/ (density_a * sos_a) * (self._pressure[0, 1] - pressure_a)
            ...
            self.mass_flow_suction_valve[0, 0] = self._volume_flow[0, 1]
            ...
            if not self._suction_no_flow[0, 0]:
                if self._volume_flow[0, 1] <= self.mass_flow_suction_valve[0, 0] <= 0.0:
                    self._suction_delta_p[0, 0] = 0.0
    ...
    def calculate_suction_valve(self, lower_pressure, upper_pressure, viscosity, density):
        ...
        if displacement <= 0:
            ...
            if self._suction_gravity_force[1, 0] + self._suction_spring_force[1, 0] + self._suction_upper_pressure_force[1, 0]\
                ...
                self._suction_cases.append('Is closed and starts to open')
            ...
            # Check: Are upper forces bigger than lower forces?
                ...
                self._suction_cases.append('Is closed and stays closed')
        ...
        # Check: Is Valve closed?
            ...
            if np.logical_and(np.abs((self._suction_contact_pressure_force[1, 0] - contact_pressure_force0) / contact_pressure_force0)\
                    >= epsilon, self._suction_no_flow[1, 0]):
                self._suction_cases.append('Close to Wall regieme')
            ...
            # Check: Close to wall regime?
                ...
                self._suction_valve_zeta[1, 0] = self.calculate_suction_zeta_value(displacement,
                                                                                   reynolds_number,
                                                                                   lower_pressure,
                                                                                   upper_pressure,
                                                                                   density)
    ...
    def calculate_discharge_dimensionless_coefficient_of_force(self, reynolds_number, displacement):
        if reynolds_number == 0.0:
            ...
            result = (self._discharge_factor_k0
                      ...
                      - self._discharge_factor_k2 * np.log(np.abs(reynolds_number)))
    ...
    def calculate_discharge_flow_force(self, dimensionless_coefficient, density, lower_pressure, upper_pressure):
        ...
        if lower_pressure - upper_pressure <= 0.0:
            result = 0.0
        return result
    ...
    def calculate_discharge_zeta_value(self, displacement, reynolds_number, density):
        ...
        self._discharge_delta_p[0, 0] = ((self.mass_flow_discharge_valve[0, 0] / (density * self._discharge_gap_area[1, 0])) ** 2)\
                                        * density / 2.0 * np.sign(self.mass_flow_discharge_valve[0, 0]) * result
        if np.logical_or(np.sign(self.mass_flow_discharge_valve[0, 0]) <= 0.0, displacement <= 3e-6):
            self._discharge_delta_p[0, 0] = 0.0
        return result
    ...
    def calculate_discharge_flow(self, no_flow):
        ...
        if not self._discharge_no_flow[0, 0]:
            if self.mass_flow_discharge_valve[0, 0] <= 0.0:
                self._discharge_delta_p[0, 0] = 0.0
    ...
    def calculate_discharge_valve(self, lower_pressure, upper_pressure, viscosity, density):
        ...
        # Check: Is Valve closed?
            ...
            if np.logical_and(np.abs(np.abs(self._discharge_contact_pressure_force[1, 0])
                      - np.abs(contact_pressure_force0)) / contact_pressure_force0\
                    >= epsilon, self._discharge_no_flow[1, 0]):#, self._discharge_valve_displacement[1, 0] <= 3e-6):
    ...
    def calculate_pump_pressure(self, bulk_modulus, density_pump):
        ...
        combined_compressibility = (1-self._alpha_air[1, 0])*density_pump / bulk_modulus + self._alpha_air[1, 0] * self.density_air / (self.pump_pressure[1, 0] * 1.2)
        combined_density = (1-self._alpha_air[1, 0])*density_pump + self._alpha_air[1, 0]*self.density_air
        #density_pump / bulk_modulus * (1 - self._alpha_air[1, 0]) + self._alpha_air[1, 0] * 1 / (
        #            self.pump_pressure[1, 0] * 1.2) * self.density_air

        if np.logical_and(self._suction_no_flow[1, 0], self._discharge_no_flow[1, 0]):

            self.pump_pressure[0, 0] = ((mass_flow_suction_valve - mass_flow_discharge_valve - (density_pump*(1-self._alpha_air[1, 0]) + self._alpha_air[1, 0] * self.density_air) *
            ...
            self.initial_p0 = self.pump_pressure[0, 0]
            self.initial_V0 = self._alpha_air[1, 0] * (self.pump_death_volume
                                            + self.piston_area * self.pump_radius * 2
                                            - self._pump_volume[1, 0]
                                            - self.suction_valve_area * self._suction_valve_displacement[1, 0]
                                            + self.discharge_valve_area * self._discharge_valve_displacement[1, 0])

            density = density_pump * (1 - self._alpha_air[1, 0]) + self._alpha_air[1, 0] * self.density_air
            self._pump_density[1, 0] = density
            self._pump_volume_velocity[0, 0] = self._pump_piston_velocity[0, 0]
            self._sos[0, 0] = density_pump / bulk_modulus*(1 - self._alpha_air[1, 0]) + self._alpha_air[1, 0] * self.density_air / (self.pump_pressure[1, 0] * 1.2)
            self._alpha_vapor[0, 0] = 0.0
            ...
            volume = (self.pump_death_volume
                      + self.piston_area * self.pump_radius * 2
                      - self._pump_volume[1, 0]
                      - self.suction_valve_area * self._suction_valve_displacement[1, 0]
                      + self.discharge_valve_area * self._discharge_valve_displacement[1, 0])
            volume_change = (self._pump_volume_change[0, 0]
                             - self.suction_valve_area * self._suction_valve_velocity[1, 0])
                             #+ self.discharge_valve_area * self._discharge_valve_velocity[1, 0])

            #if self.pump_pressure[1, 0] <= self.fluid.vapor_pressure():

             #   density_liquid = 997.2476
             #   density_vapor = 0.0219
             #   mixed_density = self._alpha_vapor[1, 0]*density_vapor + (1.0 - self._alpha_vapor[1, 0])*density_liquid
             #   self._alpha_vapor[0, 0] = (mixed_density + (density_pump*self._suction_gap_flow[1, 0])-mixed_density*volume_change/volume*self._delta_t-density_liquid)/(density_vapor-density_liquid)
                #self._suction_gap_flow[0, 0] = np.sqrt((self._pressure[1, 0]-self.pump_pressure[0, 0])*2.0*self._suction_gap_area[1, 0]**2/(density_pump*self._suction_valve_zeta[2, 0]))
             #   self._suction_gap_flow[0, 0] = (volume_change*mixed_density - self._suction_valve_velocity[1, 0] * self.suction_valve_area*mixed_density)/density_pump
             #   self._suction_delta_p[0, 0] = self._pressure[1, 0]-self.pump_pressure[0, 0]

             #   if self._alpha_vapor[0, 0] <= 0.0:
             #       self._alpha_vapor[0, 0] = 0.0
             #       self.pump_pressure[0, 0] = self.fluid.vapor_pressure()+2.0
             #   self.cavitation_counter = True

            #else:
            density = density_pump*(1 - self._alpha_air[1, 0]) + self._alpha_air[1, 0] * self.density_air
            combined_compressibility = (1-self._alpha_air[1, 0])*density_pump / bulk_modulus + self._alpha_air[1, 0] * self.density_air / (self.pump_pressure[1, 0] * 1.2)
            compressibility = density_pump / bulk_modulus
            #velocity = self._volume_flow[1, 1]/self._area[0]
            #new_velocity = self._delta_t / (density_pump * volume) * (-(self.mass_flow_suction_valve[1, 0]*self._volume_flow[1, 1]/self._area[0]-density_pump*volume_change)*velocity - density_pump*velocity*volume_change-self.mass_flow_suction_valve[1, 0]*velocity) + velocity
            #self._suction_gap_flow[0, 0] = new_velocity * self.piston_area - self._suction_valve_velocity[1, 0]*self.suction_valve_area
            #print(self._suction_gap_flow[0, 0])
            volume_change = (self._pump_volume_change[0, 0]
                             - self.suction_valve_area * self._suction_valve_velocity[1, 0])

            self._suction_gap_flow[0, 1] = (combined_compressibility * (self.pump_pressure[1, 0] - self.pump_pressure[2, 0])/ self._delta_t * volume + density * volume_change) / density
            self._suction_gap_flow[0, 0] = (self._pump_volume_change[0, 0] - self._suction_valve_velocity[
                1, 0] * self.suction_valve_area)
            self._suction_gap_flow[0, 2] = volume_change

            #print("Standard")
            #print(self._suction_gap_flow[0, 0])
            #print("Mit Kompressibilität")
            #print(self._suction_gap_flow[0, 1])
            #print("Mit Volumechange")
            #print(self._suction_gap_flow[0, 2])
            #print("delta P")
            ...
            dpdt = (self.pump_pressure[1, 0]-self.pump_pressure[2, 0]) /self._delta_t*0.0
            #dpdt = self.pump_pressure[1, 0]-self.pump_pressure[2, 0]

            self._pump_density[1, 0] = density

            #print(density*x*dpdt*volume)
            #print(density)
            #print(x)
            #print(dpdt)
            #self._suction_gap_flow[0, 0] = (density*x*dpdt*volume+density*volume_change)/density_pump
            #self._suction_gap_flow[0, 0] = (self._pump_volume_change[0, 0] - self._suction_valve_velocity[1, 0] * self.suction_valve_area)*density/density_pump
            #self._suction_gap_flow[0, 0] = (self._pump_volume_change[0, 0]*density)
            self.cavitation_counter = False
            self._alpha_vapor[0, 0] = 0.0
    ...
    def calculate_next_inner_iteration(self, iteration: int) -> bool:
        ...
        self.calculate_density_air()
        self.calculate_alpha_air()
